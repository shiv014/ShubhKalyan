...
export async function POST(req) {
  try {
    ...
    const { templateId, brideName, groomName, eventDate, venue } = await req.json();
    ...
    await db.run(
      `UPDATE events 
       SET template_id = ?, bride_name = ?, groom_name = ?, event_date = ?, venue = ?
       WHERE user_id = ?`,
      [templateId, brideName, groomName, eventDate, venue, user.id]
    );
